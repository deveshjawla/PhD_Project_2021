...
features = readdlm(PATH*"/thrity_imp_features.csv", ',', Float64)
...
function feedforward(θ::AbstractVector)
    W0 = reshape(θ[1:270], 9, 30)
    b0 = θ[271:279]
    W1 = reshape(θ[280:306], 3, 9)
    b1 = θ[307:309]
    W2 = reshape(θ[309:311], 1, 3)
    b2 = θ[312:312]
...
@model bayesnn(x, y) = begin
    θ ~ MvNormal(zeros(312), sigma .* ones(312))
    nn = feedforward(θ)
    ŷ = nn(x)
    for i = 1:length(y)
        y[i] ~ Bernoulli(ŷ[i])
...
writedlm(PATH*"/thrity_imp_features.txt", [elapsed, mcc, f1, acc, fpr, fnr, tpr, tnr, prec, recall],',')
